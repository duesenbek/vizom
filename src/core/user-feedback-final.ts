...
/**
 * Abort Controller for Request Cancellation - Completely Fixed
 */
export class AbortControllerImplementation {
  private _aborted = false;
  private abortCallbacks: (() => void)[] = [];
  private _signal: AbortSignal;

  constructor(public id: string) {
    // Create a proper AbortSignal that matches the interface
    this._signal = {
      aborted: false,
...
export class ThinkingIndicatorManager {
  private indicators: Map<string, ThinkingIndicator> = new Map();
  private animationFrames: Map<string, number> = new Map();

  /**
   * Show thinking indicator
   */
  show(requestId: string, config: {
    message?: string;
    steps?: string[];
   ...
   */
  update(requestId: string, updates: Partial<ThinkingIndicator>): void {
   ...
   * Get indicator state
   */
  getIndicator(requestId: string): ThinkingIndicator | undefined {
    return this.indicators.get(requestId);
  }

  private startAnimation(requestId: string): void {
    const indicator = this.indicators.get(requestId);
    if (!indicator) return;

    const animate = (): void => {
      if (!this.indicators.has(requestId)) return;
      ...
      indicator.currentStepIndex = Math.min(
        Math.floor(stepProgress),
        indicator.steps.length - 1
      );
      ...
      if (indicator.estimatedTimeRemaining) {
        const elapsed = Date.now() - (indicator as any).startTime;
        indicator.estimatedTimeRemaining = Math.max(0, 
          (indicator.estimatedTimeRemaining as number) - elapsed
        );
      }
    ...
    // Store start time
    (indicator as any).startTime = Date.now();
    animate();
  }

  private stopAnimation(requestId: string): void {
  ...
  private renderIndicator(indicator: ThinkingIndicator): void {
  ...
  private createIndicatorContainer(requestId: string): HTMLElement {
  ...
  private removeIndicator(requestId: string): void {
...
export class PartialResultsManager {
  ...
  private resultCallbacks: Map<string, (result: PartialResult) => void> = new Map();
   ...
   */
  onPartialResult(requestId: string, callback: (result: PartialResult) => void): void {
   ...
   */
  getPartialResults(requestId: string): PartialResult[] {
  ...
  private createResultsContainer(requestId: string): HTMLElement {
    const container = document.createElement('div');
  ...
  private removePartialResultsContainer(requestId: string): void {
  ...
  private addPartialResultsStyles(): void {
    const styles = `
...
/**
 * User Feedback Service - Completely Fixed Version
 */
export class UserFeedbackService {
  private config: FeedbackConfig;
  private thinkingManager: ThinkingIndicatorManager;
  private partialResultsManager: PartialResultsManager;
  ...
  constructor(config: Partial<FeedbackConfig> = {}) {
    ...
    (window as any).userFeedback = this;
  }
  ...
  startRequest(
    ...
    const state: RequestState = {
      ...
      partialResults: [],
      cancelToken: new AbortControllerImplementation(requestId) as AbortToken
    };
   ...
   * Add partial result
   */
  addPartialResult(requestId: string, data: any, confidence: number = 0.5): void {
    if (!this.config.showPartialResults) return;
   ...
   */
  private getDefaultSteps(type: 'chart' | 'analysis' | 'export'): string[] {
    switch (type) {
      case 'chart':
        return [
          'Analyzing your data...',
   ...
   */
  closePartialResults(requestId: string): void {
    this.partialResultsManager.clearPartialResults(requestId);
  }
