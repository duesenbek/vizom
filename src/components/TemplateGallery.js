...
class TemplateGallery {
  ...
  getTemplateData() {
    return [
      ...
      {
        id: 'd3-gallery',
        title: 'D3 Gallery Playground',
        category: 'd3',
        description: 'Explore 170+ Observable D3 examples: animation, interaction, analysis, hierarchies, maps, and more. Curated jumping-off points with API index links and ready-to-fork starter code.',
        badge: 'free',
        chartType: '🌀 D3 Showcase',
        uses: 312,
        rating: 4.9,
        setupTime: 'Instant launch',
        preview: {
          gradient: 'from-slate-900 via-indigo-700 to-cyan-500',
          label: 'D3',
          code: 'Gallery 01'
        },
        useCases: [
          'Interactive visualization prototyping',
          'Learning D3 layouts and behaviors',
          'Building Observable-inspired storytelling'
        ],
        features: {
          setupTime: 'Instant launch',
          exportFormats: 'SVG, PNG (via export helpers)',
          customization: 'Code-first • Full control',
          dataSources: 'CSV, JSON, GeoJSON, TopoJSON'
        }
      }
    ];
  }
  ...
  useTemplate(templateId) {
    ...
    if (templateId === 'd3-gallery') {
      try {
        sessionStorage.setItem('vizom:d3-gallery', 'true');
        sessionStorage.setItem('templatePrompt', 'D3 Gallery Playground activated. Choose any Observable D3 example from the gallery panel to preview it instantly or open it on Observable.');
      } catch (error) {
        console.warn('Unable to persist D3 gallery selection:', error);
      }
    } else {
      try {
        sessionStorage.removeItem('vizom:d3-gallery');
      } catch (error) {
        console.warn('Unable to clear D3 gallery flag:', error);
      }
    }
