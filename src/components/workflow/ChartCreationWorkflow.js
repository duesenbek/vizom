...
export class ChartCreationWorkflow {
  ...
  /**
   * Create the chart creation workflow definition
   */
  createWorkflow() {
    return {
      id: 'chart-creation',
  ...
  /**
   * Check if automation should be applied
   */
  ...
  getProgress() {
    const { currentStep } = this.currentState;
    return (currentStep / this.workflow.steps.length) * 100;
  }
